...
public:
	...
	/*
		Может быть вызван из неопределённого потока, но сессия должна быть создана именно в главном потоке приложения
		для захвата нужно планировщика (планировщик может быть так же передан в качестве параметра - это уже специфично для каждой реализации).
	*/
